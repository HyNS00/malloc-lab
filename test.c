...
int mm_init(void){ // 처음에 heap을 시작할 때 0부터 시작. 완전 처음.(start of heap)
    ...
    /* 변수를 초기화하는 작업, 가용 리스트에서 검색을 시작할 시, 위치를 힙의 처음으로 설정한다.
    이는 next fit할당 방식에서 다음 할당 시에 검색을 시작할 위치를 설정하는 역할 > 초기에는 힙의 시작 주소를 가리킴 처음 할당 시 힙의 처음부터 검색 시작*/
    return 0;
}
...
void *mm_malloc(size_t size){ // 가용 리스트에서 블록 할당 하기
    size_t asize; // 블록 사이즈 권장
    size_t extendsize; // heap에 맞는 fit이 없으면 확장하기 위한 사이즈
    char *bp;

    if (size == 0) return NULL;
    if (size <= DSIZE){
        asize = 2*DSIZE; // 헤더와 푸터 합쳐서 8바이트, 16/1이 들어가야하므로    
    } else{
...
static void *find_fit(size_t asize){ // first fit 검색을 수행
    ...
    while (GET_SIZE(HDRP(bp)) > 0){
        if (!GET_ALLOC(HDRP(bp)) && (asize <= GET_SIZE(HDRP(bp)))) {
            last_bp = bp; // 다음 할당 시 검색을 시작할 위치를 업데이트
            return bp;
        }
        bp = NEXT_BLKP(bp);
        if(bp == (char*)heap_listp) // 힙의 끝에 도달했을 때 다시 처음부터 검색하도록 조정
            bp = last_bp;
    }
